...
import { UnitNumbers, UnitOccupancyOption, UnitOccupancyStatus, UnitPrimaryUses } from 'src/app/core/constants';
import {
  DialogService, DynamicDialogComponent, DynamicDialogRef,
...
export class EditUnitModalComponent implements OnInit {
  ...
  async fetchBuildingDetail(buildingId) {
    this.buildingDetail = await this.buildingDataService.GetBuildingDetailsByBuildingId(buildingId).toPromise()
    console.log("Building details", this.buildingDetail)
    this.totalFloorCount = this.buildingDetail.atticCount + this.buildingDetail.jamthogCount + this.buildingDetail.floorCount + this.buildingDetail.stiltCount + this.buildingDetail.basementCount
    this.floorLevelOption = Array.from({ length: this.totalFloorCount }, (_, i) => i + 1)
  }
  ...
  async createUnitAndDetails() {
    ...
    const createdUnit = await this.unitdataService.CreateUnit(this.unit).toPromise();
    console.log(createdUnit)
    this.unit.qrUuid = createdUnit['id'].toString();
    const resp = await this.unitdataService.UpdateUnitByUnitId(this.unit, createdUnit['id']).toPromise();
    if (resp) {
    ...
    const resp2 = await this.unitdataService.CreateUnitDetail(this.unitDetail).toPromise();
    if (resp2) {
      this.messageService.add({
        severity: 'Success',
  ...
  async updateUnit() {
    ...
    const resp = await this.unitdataService.UpdateUnitByUnitId(this.unit, this.instance.data.unit.id).toPromise()
    if (resp) {
      this.messageService.add({
        severity: 'Success',
  ...
  async updateUnitDetail() {
    ...
    const resp = await this.unitdataService.UpdateUnitDetails(this.instance.data.unit.id,this.unitDetail).toPromise()
    if (resp) {
      this.messageService.add({
        severity: 'Success',
